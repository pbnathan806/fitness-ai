...
def test_get_setting_succeeds_for_super_admin():
    repository = FakeApplicationSettingRepository()
    setting = _make_setting(key="physical_assessment_overdue_days", value="14")
    repository.seed(setting)
    _override_dependencies(repository, uuid.uuid4(), RoleName.SUPER_ADMIN)
    test_client = TestClient(app)

    response = test_client.get("/api/v1/application-settings/physical_assessment_overdue_days")
...
def test_update_setting_succeeds_for_super_admin():
    ...
    response = test_client.patch(
        "/api/v1/application-settings/physical_assessment_overdue_days", json={"value": "21"}
    )
...
def test_update_setting_rejects_empty_value():
    ...
    response = test_client.patch(
        "/api/v1/application-settings/physical_assessment_overdue_days", json={"value": ""}
    )
